...
<div class=container-fluid id=add>
	...
	<form id="form" name="aOcProd" method="post" action="<%=response.encodeURL("ProductModifyController")%>" enctype="multipart/form-data" onsubmit="event.preventDefault(); validateForm(this)">
    	<div>
        	...
        	<p id="errDesc" class="errTextInterior"></p>
        	<label>Prezzo: </label>
        	<input type="number" min="0" value="1" step="1" name="prezzo"><br>
        	<label>Quantità: </label>
        	<input type="number" min="0" value="1" name="quantita"><br>
        	...
        	<input id="desc" type="text" name="desc"><br>
        	<label>Prezzo: </label>
        	<input id="prezzo" type="number" min="1" value="1" step=".01" name="prezzo" name="prezzo"><br>
        	<label>Quantità: </label>
        	<input id="quantita" type="number" min="1" value="1" name="quantita"><br>
...
<script>
	...
	function validateForm(obj) {
		...
		if ( !validateDesc(descProd) ) {
			...
			document.getElementById('errDesc').innerHTML="Descrizione non valida (Solo lettere, spazio accettato)";
		} else {
